...
DEBUG_LOG_END
  Simulator::Init()->Schedule(0.,
                              &RadioChannel::ScheduleRx,
                              this,
                              p,
                              txSignal,
                              src);
...
void RadioChannel::ScheduleRx(shared_ptr<PacketBurst> p, TransmittedSignal *txSignal, NetworkNode *src)
{
DEBUG_LOG_START_1(SIM_ENV_TEST_DEVICE_ON_CHANNEL)
...
for (auto dst : *GetDevices())
{
  ...
  if (dst->GetNodeType() != NetworkNode::TYPE_MULTICAST_DESTINATION)
  {
    ...
    CartesianCoordinates* srcPos = src->GetMobilityModel()->GetAbsolutePosition();
    CartesianCoordinates* dstPos = dst->GetMobilityModel()->GetAbsolutePosition();
    double propagationDelay = (srcPos->GetDistance3D (dstPos))/(299792458.0);

    //Trigger StartRx on the destination Phy after the propagation delay
    Simulator::Init()->Schedule(propagationDelay,
                              &Phy::StartRx,
                              dst->GetPhy(),
                              p->Copy(),
                              rxSignal);
