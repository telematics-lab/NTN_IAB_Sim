...
{
  if (configuration < 0 && configuration > 11)
    {
...
void FrameManager::ResourceAllocation(void)
{
  ...
  for (auto record : *records)
  {
      ...
      DEBUG_LOG_START_1(SIM_ENV_FRAME_MANAGER_DEBUG)
      cout << " FRAME_MANAGER_DEBUG: RBs allocation for gNB " << record->GetIDNetworkNode() << endl;
      DEBUG_LOG_END
      ...
      else
      {
        ...
        if (GetSubFrameType(GetNbSubframes()) == 0)
        {
          ...
          DEBUG_LOG_END
          if(record->GetNodeType() == NetworkNode::TYPE_GNODEB)
        ...
        }else if (GetSubFrameType(GetNbSubframes()) == 3)
        {
          DEBUG_LOG_START_1(SIM_ENV_FRAME_MANAGER_DEBUG)
          cout << " FRAME_MANAGER_DEBUG: SubFrameType = SUBFRAME_FOR_DOWNLINK_IAB " << endl;
          DEBUG_LOG_END
          if(record->GetNodeType() == NetworkNode::TYPE_RELAY_NODE)
          Simulator::Init()->Schedule(0.0, &GNodeB::DownlinkResourceBlockAllocation, record);
        }
        else if (GetSubFrameType(GetNbSubframes()) == 4)
        {
          DEBUG_LOG_START_1(SIM_ENV_FRAME_MANAGER_DEBUG)
          cout << " FRAME_MANAGER_DEBUG: SubFrameType = SUBFRAME_FOR_UPLINK_IAB " << endl;
          DEBUG_LOG_END
          if(record->GetNodeType() == NetworkNode::TYPE_RELAY_NODE)
          Simulator::Init()->Schedule(0.0, &GNodeB::UplinkResourceBlockAllocation, record);
        }
        else if (GetSubFrameType(GetNbSubframes()) == 5)
        {
          //Da testare ma dovrebbe funzionare...
          DEBUG_LOG_START_1(SIM_ENV_FRAME_MANAGER_DEBUG)
          cout << " FRAME_MANAGER_DEBUG: SubFrameType = SUBFRAME_FOR_BACKHAUL_FDD " << endl;
          DEBUG_LOG_END
          if(record->GetNodeType() == NetworkNode::TYPE_GNODEB)
          Simulator::Init()->Schedule(0.0, &GNodeB::UplinkResourceBlockAllocation, record);
          Simulator::Init()->Schedule(0.0, &GNodeB::DownlinkResourceBlockAllocation, record); 
  ...
  {
      DEBUG_LOG_START_1(SIM_ENV_FRAME_MANAGER_DEBUG)
      cout << " FRAME_MANAGER_DEBUG: RBs allocation for gNB " << record_2->GetIDNetworkNode() << endl;
      DEBUG_LOG_END
