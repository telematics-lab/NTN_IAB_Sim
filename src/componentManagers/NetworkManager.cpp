...
GNodeB*
NetworkManager::CreateSatGnodeb (bool isLeo, int id,
                              Cell* cell,
                              double pos_X, double pos_Y, double pos_Z,
                              int numTxAntennas, int numRxAntennas,
                              RadioChannel *dlCh, RadioChannel *ulCh,
                              BandwidthManager *bm, int NumSat)
{
    if (isLeo)
  {
  GNodeB* gnb = new GNodeB (id, cell, pos_X, pos_Y, pos_Z, NumSat, bm->GetNBIoTenabled()); 
  gnb->GetPhy ()->SetTxAntennas (numTxAntennas);
  gnb->GetPhy ()->SetRxAntennas (numRxAntennas);
  gnb->GetPhy ()->SetDlChannel (dlCh);
  gnb->GetPhy ()->SetUlChannel (ulCh);
...
IABNode*
NetworkManager::CreateIABNode (int id,
                              Cell* cell,
                              double pos_X, double pos_Y,
                              int numTxAntennas, int numRxAntennas,
                              RadioChannel *dlCh, RadioChannel *ulCh,
                              BandwidthManager *bm)
{
  IABNode *iab = new IABNode(id, cell, pos_X, pos_Y); // GNodeB (1, cell);
  iab->GetPhy()->SetTxAntennas(numTxAntennas);
  iab->GetPhy()->SetRxAntennas(numRxAntennas);
  iab->GetPhy()->SetDlChannel(dlCh);
  iab->GetPhy()->SetUlChannel(ulCh);
  iab->GetPhy()->SetBandwidthManager(bm);
  ulCh->AddDevice(iab);
  GetGNodeBContainer()->push_back(iab);
  return iab;
}

IABNode*
NetworkManager::CreateSatIABNode (bool isLeo, int id,
                              Cell* cell,
                              double pos_X, double pos_Y, double pos_Z,
                              int numTxAntennas, int numRxAntennas,
                              RadioChannel *dlCh, RadioChannel *ulCh,
                              BandwidthManager *bm, int NumSat)
{
  if (isLeo)
  {
    IABNode *iab = new IABNode(id, cell, pos_X, pos_Y, pos_Z, NumSat, bm->GetNBIoTenabled());
    iab->GetPhy()->SetTxAntennas(numTxAntennas);
    iab->GetPhy()->SetRxAntennas(numRxAntennas);
    iab->GetPhy()->SetDlChannel(dlCh);
    iab->GetPhy()->SetUlChannel(ulCh);
  ...
  {
    cout << "Error: GEO IAB node not supported." << endl;
    exit(1);
  }
}
...
UserEquipment*
NetworkManager::CreateSatUserEquipment (int id,
                                     double pos_X, double pos_Y, double speed, double speedDirection, int numTxAntennas, int numRxAntennas,
                                     Cell* cell, GNodeB* gnb)
{
  UserEquipment* ue = new UserEquipment (id,
  ...
  ue->SetTimePositionUpdate (0.05); // Trigger for mobility
  ...
  ChannelRealization* c_dl = new ChannelRealization (gnb, ue, ChannelRealization::CHANNEL_MODEL_SATELLITE);
  gnb->GetPhy()->GetDlChannel ()->GetPropagationLossModel ()->AddChannelRealization (c_dl);

  ChannelRealization* c_ul = new ChannelRealization (ue, gnb, ChannelRealization::CHANNEL_MODEL_SATELLITE);
  gnb->GetPhy()->GetUlChannel ()->GetPropagationLossModel ()->AddChannelRealization (c_ul);
...
UserEquipment*
NetworkManager::CreateMobileTermination (int id,
                                     double pos_X, double pos_Y, double speed, double speedDirection, int numTxAntennas, int numRxAntennas,
                                     Cell* cell, GNodeB* gnb, IABNode* iab)
{
  UserEquipment* ue = new UserEquipment (id,
                                         pos_X, pos_Y, speed, speedDirection,
                                         cell, gnb,
                                         0,
                                         Mobility::CONSTANT_POSITION, iab);
...
UserEquipment*
NetworkManager::CreateSatMobileTermination (int id,
                                     double pos_X, double pos_Y, double speed, double speedDirection, int numTxAntennas, int numRxAntennas,
                                     Cell* cell, GNodeB* gnb, IABNode* iab)
{
  UserEquipment* ue = new UserEquipment (id,
...
{
  if ( target->GetNodeType() == NetworkNode::TYPE_GNODEB ||  target->GetNodeType() == NetworkNode::TYPE_RELAY_NODE)
    {
      return true;
    }
...
DEBUG_LOG_END
  ...
  if ( (target->GetNodeType() == NetworkNode::TYPE_RELAY_NODE && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_RELAY_NODE) ||
      (target->GetNodeType() == NetworkNode::TYPE_GNODEB && ue->GetTargetNode ()->GetNodeType() == NetworkNode::TYPE_GNODEB) ||
