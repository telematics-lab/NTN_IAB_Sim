...
#include "../protocolStack/mac/packet-scheduler/single-uplink-packet-scheduler.h"
               ...
               int nSat, bool isNBIoT) // Constructor for satellite scenario
{
  ...
  else
  {
    ...
    SetMobilityModel(m);
    m->SetDevice(this);
    ((SatelliteMovement *)m)->SetIsLeo(false);
    delete position;
...
GNodeB::UserEquipmentRecord::UserEquipmentRecord (UserEquipment *UE)
{
  ...
  for (int i = 0; i < nbRbs; i++ )
    {
      m_cqiFeedback.push_back (10); //Default value that throws an MCS of 18
    }
  ...
  for (int i = 0; i < nbRbs; i++ )
    {
      m_uplinkChannelStatusIndicator.push_back (10); //Default value that throws an MCS of 18
    }
...
void
GNodeB::UserEquipmentRecord::SetUplinkChannelStatusIndicator (vector<int> vet)
{
...
GNodeB::SetULScheduler (ULSchedulerType type)
{
  ...
  switch (type)
    ...
    case GNodeB::ULScheduler_TYPE_NB_IOT_ROUNDROBIN:
      ...
      mac->SetUplinkPacketScheduler (scheduler);
      break;
    case GNodeB::ULScheduler_TYPE_SINGLE:
      scheduler = new SingleUplinkPacketScheduler ();
      scheduler->SetMacEntity (mac);
      mac->SetUplinkPacketScheduler (scheduler);
      break;
...
*/
void
GNodeB::SetCqiManager (CqiManager *cm)
{
  m_cqiManager = cm;
}

CqiManager*
GNodeB::GetCqiManager (void)
{
  return m_cqiManager;
}
