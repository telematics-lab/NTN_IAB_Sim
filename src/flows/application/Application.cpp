...
void Application::Start()
{
  ...
  if (GetSource()->GetNodeType() == NetworkNode::TYPE_UE)
  {
    ...
    m_radioBearer->GetSource()->GetProtocolStack()->GetRrcEntity()->AddRadioBearer(m_radioBearer);
           ...
           GetSource()->GetNodeType() == NetworkNode::TYPE_RELAY_NODE ||
           GetSource()->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION)
  {
    if (GetDestination()->GetNodeType() == NetworkNode::TYPE_UE)
    {
      ...
      m_radioBearer = new RadioBearer();
      m_radioBearer->GetRlcEntity()->SetRlcEntityIndex(GetApplicationID());
      m_radioBearer->GetRlcEntity()->SetDevice(ue->GetTargetNode());
  ...
  if (GetDestination()->GetNodeType() == NetworkNode::TYPE_UE)
  {
    if (dynamic_cast<UserEquipment *>(GetDestination())->GetIABNode() != nullptr)
      isMT = true; // Destination is an MT attached to an IAB, so we need relay receiver as application sink
  }

  if (GetDestination()->GetNodeType() == NetworkNode::TYPE_RELAY_NODE || isMT)
  {
    m_relayReceiver = new RelayReceiver();
    m_relayReceiver->SetClassifierParameters(GetClassifierParameters());
    m_relayReceiver->SetSourceApplication(this);
  }
  else
  {
  ...
  if (m_applicationSink == nullptr)
  {
    m_bearerSink->SetRelay(m_relayReceiver);
  }
  else
  {
  ...
  else if (GetSource()->GetNodeType() == NetworkNode::TYPE_GW || GetSource()->GetNodeType() == NetworkNode::TYPE_GNODEB || GetSource()->GetNodeType() == NetworkNode::TYPE_RELAY_NODE || GetSource()->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION)
  {
    UserEquipment *ue = (UserEquipment *)GetDestination();
  ...
  m_radioBearer->GetDestination()->GetProtocolStack()->GetRrcEntity()->AddRadioBearerSink(m_bearerSink);
    if (m_applicationSink == nullptr)
  {
    m_relayReceiver->SetRadioBearerSink(m_bearerSink);
  }
  else
  {
...
void Application::enqueue(Packet *p)
{
  if (GetDestination()->GetNodeType() == NetworkNode::TYPE_MULTICAST_DESTINATION)
  {
...
void Application::SetTransmittingNodes(vector<NetworkNode *> nodes)
{
  m_transmittingNodes = nodes;
}

void Application::AddTransmittingNode(NetworkNode *node)
{
...
void Application::SetRadioBearers(vector<RadioBearer *> bearers)
{
  m_radioBearers = bearers;
}

void Application::AddRadioBearer(RadioBearer *bearer)
{
...
void Application::SetExpirationTime(double time)
{
  m_stopTime = time;
}
...
void Application::Trace(Packet *p)
{
  ...
  if (!_APP_TRACING_)
    return;
  cout << "NODE " << GetSource()->GetIDNetworkNode();
  if (m_applicationType == Application::APPLICATION_TYPE_RELAY_SENDER)
  {
    x = 0;
    /*
     * Trace format:
     *
     * NODE_ID TX-RELAY   APPLICATION_TYPE   BEARER_ID  SIZE   SRC_ID   DST_ID   TIME
     */
    cout << " TX-RELAY";
  ...
  else
  {
     ...
     * Trace format:
     *
     * NODE_ID TX   APPLICATION_TYPE   BEARER_ID  SIZE   SRC_ID   DST_ID   TIME
     */
    cout << " TX";
...
int 
Application::GetID(void) { return m_id; }

void 
Application::SetID(int id) { m_id = id; }
