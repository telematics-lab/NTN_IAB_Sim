  ...
  if (m_channelsForTx.size () > 0)
    {
      ...
      int maxChannels = values.at(0).size();
...
UePhy::StartRx (shared_ptr<PacketBurst> p, ReceivedSignal* rxSignal)
{
  ...
  //COMPUTE THE SINR
  vector< vector<double> > rxSignalValues = rxSignal->GetValues();
  vector< vector<float> > rxSignalPhases = rxSignal->GetPhases();
  ...
  if (GetInterference () != nullptr)
    {
      ...
      if(interference>0){
        cout << "WARNING: THERE IS AN INTERFERENCE" << endl;
      }
    }
  else
    {
  ...
  {
      if ((FrameManager::Init()->MbsfnEnabled() == true && FrameManager::Init()->isMbsfnSubframe() == true) || FrameManager::Init()->MbsfnEnabled() == false)
      {                 
          txMode = 1;
      }
  }
  ...
  if ((ue->GetCqiManager()->NeedToSendFeedbacks() == false && nbOfRxSubChannels == 0 && !(FrameManager::Init()->MbsfnEnabled() == true && FrameManager::Init()->isMbsfnSubframe() == true))  
      || (FrameManager::Init()->MbsfnEnabled() == true && FrameManager::Init()->isMbsfnSubframe() == true && (isMbsfnSignal == false || ue->GetMulticastDestination() == nullptr)))
  {
      return;
  }
         ...
         for (auto power : rxSignalValues.at(0))
         {
           if (power != 0.)
           {
             ...
             if (satScenario)
             {
               ...
               m_sinrForCQI.push_back(power - GetSatelliteSensitivity(scs));
             }
             else
             {
               m_sinrForCQI.push_back(power - noise_interference /*- DL_INTERFERENCE*/);
             }
           }
         }
      }
...
DEBUG_LOG_END
  ...
  if (GetErrorModel() != nullptr && p->GetNPackets() > 0)
  {
    if (satScenario)
    {
      ...
      phyError = GetErrorModel()->CheckForPhysicalError(m_channelsForRx, m_mcsIndexForRx, sinrForBLER_2);

      if (_PHY_TRACING_)
      {

        CartesianCoordinates *uePos = ue->GetMobilityModel()->GetAbsolutePosition();
        CartesianCoordinates *gnbPos = gnb->GetMobilityModel()->GetAbsolutePosition();
        double DIST_ = 0.0;
        if (nbOfRxSubChannels > 0)
        {
          int MCS_ = m_mcsIndexForRx.at(0);
          // int TBS_ = ue->GetProtocolStack()->GetMacEntity()->GetAmcModule()->GetTBSizeFromMCS(MCS_);
          int TBS_ = ue->GetProtocolStack()->GetMacEntity()->GetAmcModule()->GetTBSize5G(MCS_, nbOfRxSubChannels);
          double EL_ = 0.0;
          if (gnb->GetMobilityModel()->GetMobilityModel() == Mobility::SATELLITE)
          {
            
            if (((SatelliteMovement *)gnb->GetMobilityModel())->GetIsLeo())
            {
              // IS LEO
              EL_ = ((SatelliteMovement *)gnb->GetMobilityModel())->GetElAngle(uePos);
            ...
            else
            {
              // IS GEO
              EL_ = ((SatelliteMovement *)gnb->GetMobilityModel())->GetElAngle(uePos);
              DIST_ = ((SatelliteMovement *)gnb->GetMobilityModel())->GetDistanceFromAngle(EL_);
            }
          }
          ...
          double TotalSNR = 0;
          for (auto channelSNR : sinrForBLER)
          {
            TotalSNR += channelSNR;
          }
          TotalSNR = (TotalSNR / sinrForBLER.size());
          cout << "UE_PHY_RX SRC " << ue->GetIDNetworkNode()
               << " DST " << gnb->GetIDNetworkNode()
               << " DISTANCE " << DIST_
               << " ELEVATION " << EL_
               << " SNR " << TotalSNR
               << " MCS " << MCS_
    ...
    else
    {
      ...
      {
        cout << "WARNING: HARQ IS WORKING (BAD)" << endl;
        // If HARQ is enabled SINR value is improved by combining with older transmissions
        if (!(FrameManager::Init()->MbsfnEnabled() == true && FrameManager::Init()->isMbsfnSubframe() == true))
        {
      ...
      if (_PHY_TRACING_)
      {
        ...
        if (nbOfRxSubChannels > 0)
        {
          ...
          cout << "UE_PHY_RX SRC " << ue->GetTargetNode()->GetIDNetworkNode()
  ...
  if (ue->GetCqiManager ()->NeedToSendFeedbacks ())
    {
      if(sinr.size()>0)
        {
          ue->GetCqiManager ()->CreateCqiFeedbacksDl (sinr);
        }
      int txMode = ue->GetTargetNodeRecord()->GetDlTxMode();
      if(txMode == 3)
        {
...
UePhy::SetTxSignalForReferenceSymbols (void)
{
    ...
    vector<int> channels = m_channelsForTx;
...
UePhy::SendReferenceSymbols (void)
{
  ...
  //  {
      gnbPhy->ReceiveReferenceSymbols (ue, GetTxSignalForReferenceSymbols ()->Copy());
      Simulator::Init()->Schedule(0.01, &UePhy::SetTxSignalForReferenceSymbols, this);
  //  }
}

vector<int> 
UePhy::GetMCSforTx(void){
