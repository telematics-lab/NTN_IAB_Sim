...
DEBUG_LOG_START_1(SIM_ENV_SCHEDULER_DEBUG)
              printf("Scheduled User = %d mcs = %d Required RB's = %d Allocated RB's= %d\n",
                     (int) scheduledUser->m_userToSchedule->GetIDNetworkNode(),
                     (int) scheduledUser->m_selectedMCS,
                     (int) requiredPRBs[selectedUser], (int) scheduledUser->m_listOfAllocatedRBs.size() );
