...
void UplinkPacketScheduler::RBsAllocation()
{
  ...
  while (s < nbOfRBs)
  {
      ...
      if (RBIsAllocated)
      {
          ...
          // Compute the total capacity, in Byte, of the remaining PRBs
          //int tbs = (amc->GetTBSizeFromMCS(mcs, availableRBs)) / 8; // bit to Byte
          int tbs = (amc->GetTBSize5G(mcs, availableRBs)) / 8; // bit to Byte
          
          if (tbs <= dataToTransmit)
          {
            // The user with highest priority take all the remaining PRBs
          ...
          {
            int rbsNeeded = 1;
            //int tbsNeeded = (amc->GetTBSizeFromMCS(mcs, rbsNeeded)) / 8;
            int tbsNeeded = (amc->GetTBSize5G(mcs, rbsNeeded)) / 8;

            while (tbsNeeded <= dataToTransmit)
            {
              rbsNeeded++;
              //tbsNeeded = (amc->GetTBSizeFromMCS(mcs, rbsNeeded)) / 8;
              tbsNeeded = (amc->GetTBSize5G(mcs, rbsNeeded)) / 8;
            }
            scheduledUser->m_transmittedData = tbsNeeded;
