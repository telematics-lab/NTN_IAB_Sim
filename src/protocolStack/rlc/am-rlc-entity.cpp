...
#include "../../flows/application/relay-receiver.h"
...
DEBUG_LOG_END
  ...
  if (thisBearer->GetApplication() == nullptr)
  {
        RelayReceiver *thisApplication = (RelayReceiver *)thisBearer->GetRelay();
  ...
  for (auto amdRecord : *m_sentAMDs)
    {
      ...
      if ((HOL > maxDelay || amdRecord->m_retx_count > MAX_AMD_RETX) && 0)
        {
