...
/*
  ...
  through a CBR Application, being in the range of a Satellite IABNode MT (ID = 3) in charge to receive packets and relays
  them via a Satellite IABNode DU (ID = 2) to a terrestrial UE (ID = 1000).
*/

//Basic configuration: ./5G-air-simulator UC1DirectDl 1 1 2 0 0 250 5 294.5 

static void UC1DirectDl(int argc, char *argv[])
{
  ...
  // CREATE CELL 1 (For Satellite IAB-node to UE)
  int idCell = 0;
  int radius = 1;           // [km]
  ...
  for (int user = 0; user < nUEs; user++)
  {
    ...
    int srcPort = 0;         // Port on the IAB Sat
