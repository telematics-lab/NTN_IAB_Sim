...
//Basic configuration: ./5G-air-simulator UC2DirectDl 1 2 45 250 5 60.0

static void UC2DirectDl(int argc, char *argv[])
{
...
for (int user = 0; user < nUEs; user++)
{
  ...
  UEs.back()->GetPhy()->SetErrorModel(errorModel);
  ...
  // INSTALL APPLICATION
  int applicationID = user + 1;
  APPs.push_back(new CBR());
  APPs.back()->SetApplicationID(applicationID);
  APPs.back()->SetSource(gnb_sat);
  APPs.back()->SetDestination(UEs.back());
