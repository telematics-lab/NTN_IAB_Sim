...
/*
  Description: inerithed from UC2RelayDl
*/

//Basic configuration: ./5G-air-simulator V_004_SATELLITE_3 1 2 45 5000000 20.0 20.0

static void V_004_SATELLITE_3(int argc, char *argv[])
{
  ...
  /*4*/ int FTPSize = atoi(argv[5]);  //250
  /*5*/ double FTPInterval = atof(argv[6]);  //5
  ...
  frameManager->SetTDDFrameConfiguration(10);
  ...
  // IAB to gNB Sat FLOW INFORMATION
  int srcPort = 0;   // Port on the IAB
  int dstPort = 100; // Port on the Sat gNB
  int srcID = 4;     // It can be assumed the ID as a L3 address, i.e., the IP address
  int dstID = 3;
  ...
  int MtTxAntennas = 1;
  int MtRxAntennas = 1;
  UserEquipment *mt = networkManager->CreateSatMobileTermination(idMt, 0, 0, speed, 0, MtTxAntennas, MtRxAntennas, cell1, gnb_sat, iab);
...
for (int user = 0; user < nUEs; user++)
{
  ...
  int srcPort = 0;         // Port on the Sat gNB
  int dstPort = 100;       // Port on the UE
  ...
  relay_sender->SetStartTime(startTime + delayIABProcedures);
  relay_sender->SetStopTime(startTime + duration + delayIABProcedures);
  APPs.push_back(relay_sender);
}
// RUN SIMULATION
