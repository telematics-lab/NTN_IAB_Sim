...
/*
  Description: inerithed from UC2RelayUl
*/

//Basic configuration: ./5G-air-simulator V_004_SATELLITE_4 1 2 45 5000000 20.0 20.0

static void V_004_SATELLITE_4(int argc, char *argv[])
{
  ...
  int dstPort = 100; // Port on the IAB Sat DU
 ...
 relay_sender_1->SetDestination(gnb_sat);
...
for (int user = 0; user < nUEs; user++)
{
  ...
  APPs.back()->SetAverageInterval(FTPInterval);
  APPs.back()->SetSize(FTPSize);
