...
static void PaperDirectSatExampleUl(int argc, char *argv[])
{
  /*
  ...
  */
  ...
  frameManager->SetTDDFrameConfiguration(1);
  ...
  BandwidthManager *spectrum = new BandwidthManager(bandwidth, bandwidth, spectrum_index, spectrum_index, true, spacing); // 20 MHz in Dl and Ul, true to abilitate TDD
...
vector<ClassifierParameters*> CPs;
vector<CBR*> APPs;
// Create UEs
for(int user=0; user<nUEs; user++){
  ...
  int UeTxAntennas = 1;
  int UeRxAntennas = 1;
  UEs.push_back(networkManager->CreateSatUserEquipment(idUe, posX_ue, posY_ue, speed, 0, UeTxAntennas, UeRxAntennas, cell, gnb));
  ...
  int dstID = 4;
  CPs.push_back(new ClassifierParameters(srcID,
                                         dstID,
                                         srcPort,
                                         dstPort,
  ...
  APPs.back()->SetClassifierParameters(CPs.back());
